  ...
  <div class="container">
    <div class="row">
      ...
      <template v-if="postsStore.isLoading">
        <div
          v-for="item in postsStore.perPage"
          :key="item"
      ...
      <template v-else>
        ...
        <p v-if="postsStore.posts.length == 0" class="text-center alert alert-info col-4 offset-4 mt-2">No posts</p>
      </template>
      <nav aria-label="Page navigation example">
        <ul class="pagination justify-content-center">
          ...
          >
            <a
              class="page-link"
              :class="{ 'bg-info': postsStore.currentPage == pageNumber }"
              href="javascript:void(0)"
              >{{ pageNumber }}</a
            >
          </li>
...
export default {
  ...
  computed: {
    ...
    pageNumbers() {
      let last_page = Math.ceil(
        this.postsStore.totalPosts / this.postsStore.perPage
      );
      let pages = [];
      for (let i = 1; i <= last_page; i++) {
        ...
        if (
          i == 1 ||
          (this.postsStore.currentPage - offset <= i &&
            this.postsStore.currentPage + offset >= i) ||
          i == this.postsStore.currentPage ||
          i == last_page
        ) {
          pages.push(i);
        } else if (
          i == this.postsStore.currentPage - (offset + 1) ||
          i == this.postsStore.currentPage + (offset + 1)
        ) {
          pages.push("...");
        }
  ...
  async mounted() {
    if (this.$route.query.search) {
      this.postsStore.searchTerm = this.$route.query.search;
    }
    await this.getPosts();
    this.postsStore.styleHashtag();
  },
  ...
  methods: {
    async getPosts() {
      ...
      try {
        ...
        setTimeout(() => {
          this.postsStore.styleHashtag(this.getPosts);
        }, 100)
      } catch (error) {
        if (error.response) {
    ...
    goBack() {
      if (this.postsStore.currentPage != 1) {
        this.postsStore.currentPage--;
        this.getPosts();
      }
    },
    goForward() {
      if (
        this.postsStore.currentPage !=
        Math.ceil(this.postsStore.totalPosts / this.postsStore.perPage)
      ) {
        this.postsStore.currentPage++;
        this.getPosts();
      }
    ...
    goToPage(pageNumber) {
      if (pageNumber != "...") {
        this.postsStore.currentPage = pageNumber;
        this.getPosts();
      }
    },
  },
