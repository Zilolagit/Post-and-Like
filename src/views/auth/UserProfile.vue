...
export default {
  ...
  async mounted() {
    if (this.$route.params.userid == this.authStore.id) {
        this.$router.push('/me')
    }
    const response = await axios.get(
      `/api/v1/blog/u/${this.$route.params.userid}/`
    );
  ...
  methods: {
    async getPosts() {
      ...
      try {
        const response = await axios.get(
          `/api/v1/blog/u/${this.$route.params.userid}/posts/?page=${this.currentPage}&limit=${this.perPage}`
        );
